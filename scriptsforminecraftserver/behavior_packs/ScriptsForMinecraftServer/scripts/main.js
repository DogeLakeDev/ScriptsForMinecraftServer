    ...
    Money = class {
      ...
      static setCached(player, balance, version = 0) {
        ...
        if (previous && version > 0 && previous.version > version) {
          debug.d(
            "MNY",
            `setCached SKIP ${player.name}: stale ver=${version} < cached=${previous.version}`
          );
          return;
        }
      ...
      static async load(player) {
        ...
        if (account) {
          ...
          debug.i(
            "MNY",
            `load ${player.name}: server bal=${balance} ver=${account.version}`
          );
      ...
      static async add(player, money) {
        ...
        if (result.ok) {
          ...
          debug.e(
            "MNY",
            `add FAIL ${player.name} ${money}: ${result.error || "unknown"}`
          );
        }
        return result.ok;
      }
...
var _Database = class _Database {
  ...
  static async delete(landId, actorId, expectedVersion, requestId) {
    ...
    if (result.balance !== void 0) {
      const { Money: Money2 } = await Promise.resolve().then(() => (init_Economy(), Economy_exports));
...
// scripts/data/ScoreboardsData.ts
