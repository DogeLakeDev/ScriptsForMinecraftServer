...
var init_HttpDB = __esm({
  "scripts/libs/HttpDB.ts"() {
    "use strict";
    import_server_net = require("@minecraft/server-net");
    import_server = require("@minecraft/server");
    HOST = "127.0.0.1";
    PORT = 3001;
    ...
    HttpDB = class _HttpDB {
      ...
      static async checkHealth() {
        for (let i = 0; i < 5; i++) {
          try {
            const res = await import_server_net.http.get(`${BASE_URL}/api/health`);
            ...
            if (i < 4) {
              ...
              await import_server.system.waitTicks(40);
            } else {
      ...
      static async request(method, path, bodyData) {
        try {
          const req = new import_server_net.HttpRequest(`${BASE_URL}${path}`);
          req.timeout = TIMEOUT;
          req.method = method;
          if (bodyData) {
          ...
          const res = await import_server_net.http.request(req);
          this.available = true;
...
var import_server2, _systemMsgHandler, Msg;
var init_Tools = __esm({
  "scripts/libs/Tools.ts"() {
    "use strict";
    import_server2 = require("@minecraft/server");
...
var import_server3, CreativeArea;
var init_CreativeArea = __esm({
  "scripts/area/CreativeArea.ts"() {
    "use strict";
    import_server3 = require("@minecraft/server");
    init_ConfigManager();
    init_Tools();
    init_Permission();
    CreativeArea = class _CreativeArea {
      ...
      registerEvents() {
        ...
        this.subscriptions.push(
          import_server3.world.afterEvents.playerSpawn.subscribe((event) => {
            if (!event.initialSpawn) return;
            import_server3.system.runTimeout(() => {
        ...
        this.subscriptions.push(
          import_server3.world.afterEvents.playerDimensionChange.subscribe((event) => {
        ...
        this.subscriptions.push(
          import_server3.world.afterEvents.entitySpawn.subscribe((event) => {
            if (!_CreativeArea.enable) return;
            if (!event.entity) return;
            if (event.entity.typeId === "minecraft:player") return;
            if (!this.creativeDims.has(event.entity.dimension.id)) return;
            try {
              if (event.cause === import_server3.EntityInitializationCause.Spawned) {
        ...
        this.subscriptions.push(
          import_server3.world.beforeEvents.playerPlaceBlock.subscribe((event) => {
            if (!_CreativeArea.enable) return;
            const player = event.player;
            if (player.getGameMode() !== import_server3.GameMode.Creative) return;
        ...
        this.subscriptions.push(
          import_server3.world.beforeEvents.playerBreakBlock.subscribe((event) => {
            if (!_CreativeArea.enable) return;
      ...
      saveScores(player) {
        ...
        if (!identity) return;
        const scores = {};
        for (const obj of import_server3.world.scoreboard.getObjectives()) {
          try {
      ...
      startTick() {
        this.tickRunIds.push(
          import_server3.system.runInterval(() => {
            if (!_CreativeArea.enable) return;
            for (const player of import_server3.world.getPlayers()) {
              if (player.getGameMode() === import_server3.GameMode.Spectator) continue;
      ...
      startBorderFastCheck() {
        this.tickRunIds.push(
          import_server3.system.runInterval(() => {
            if (!_CreativeArea.enable) return;
            for (const player of import_server3.world.getPlayers()) {
              if (player.getGameMode() !== import_server3.GameMode.Creative) continue;
...
var import_server4, Peace;
var init_Peace = __esm({
  "scripts/area/Peace.ts"() {
    "use strict";
    import_server4 = require("@minecraft/server");
    init_ConfigManager();
    init_Tools();
    Peace = class _Peace {
      constructor() {
      ...
      registerEvents() {
        ...
        this.entitySpawnSub = import_server4.world.afterEvents.entitySpawn.subscribe((event) => {
          if (!this.enable) return;
          try {
            if (event.cause === import_server4.EntityInitializationCause.Spawned) {
...
var import_server5, DogeChat;
var init_DogeChat = __esm({
  "scripts/chat/DogeChat.ts"() {
    "use strict";
    import_server5 = require("@minecraft/server");
    init_Tools();
    init_Money();
    init_HttpDB();
    init_Permission();
    ...
    DogeChat = class _DogeChat {
      ...
      static async ensureDefaultChannels() {
        ...
        for (let i = 0; i < 5; i++) {
          ...
          if (i < 4) {
            await import_server5.system.waitTicks(40);
            continue;
          }
      ...
      static getOnlineCount(channelId) {
        let count = 0;
        for (const p of import_server5.world.getPlayers()) {
      ...
      static async ensurePrivateChannel(idA, idB) {
        ...
        const existing = await getChannel(channelId);
        if (existing) return existing;
        const nameB = import_server5.world.getPlayers().find((p) => p.id === idB)?.name ?? idB;
        const channel = {
          id: channelId,
          name: `\u4E0E ${nameB} \u7684\u79C1\u804A`,
          type: "private",
      ...
      static startBridgePolling(bridgeChannelId) {
        ...
        this._lastBridgeFetch = Date.now();
        this._bridgePollId = import_server5.system.runInterval(async () => {
          try {
...
var import_server6, moduleGuard, Command;
var init_Command = __esm({
  "scripts/libs/Command.ts"() {
    "use strict";
    import_server6 = require("@minecraft/server");
    init_Permission();
    init_Tools();
    init_DebugLog();
    moduleGuard = () => true;
...
`;
      ...
      static registerScriptEvent() {
        import_server6.system.afterEvents.scriptEventReceive.subscribe(
          (event) => {
...
var import_server7, descriptors, cleanups, booted, lastEnabled, worldLoaded, ModuleRegistry;
var init_ModuleRegistry = __esm({
  "scripts/libs/ModuleRegistry.ts"() {
    "use strict";
    import_server7 = require("@minecraft/server");
    init_ConfigManager();
    init_ModuleKeys();
    init_Command();
    descriptors = [];
...
var import_server8, ConfigManager;
var init_ConfigManager = __esm({
  "scripts/libs/ConfigManager.ts"() {
    "use strict";
    import_server8 = require("@minecraft/server");
    init_HttpDB();
    init_Tools();
    init_CreativeArea();
    init_Peace();
    ...
    ConfigManager = class {
      ...
      static startPolling(intervalTicks = 72e3) {
        import_server8.system.runInterval(() => this._poll(), intervalTicks);
      }
      /**
       * 快速信号检查（每 2 秒），检测 _reload_signal → 立即全量重载
       */
      static startFastPoll(intervalTicks = 40) {
        import_server8.system.runInterval(() => this._fastPoll(), intervalTicks);
      }
      static isEnabled(module2) {
        if (!this._ready) return false;
        return this.cache.modules.get(module2) ?? false;
      }
      ...
      static getAreas(module2) {
        return this.cache.areas.filter((a) => a.module === module2);
      }
      ...
      static async reloadModule(module2) {
...
var import_server9, Permission;
var init_Permission = __esm({
  "scripts/libs/Permission.ts"() {
    "use strict";
    import_server9 = require("@minecraft/server");
    init_ConfigManager();
    init_Command();
    init_Tools();
    Permission = class {
      ...
      static getPermission(player) {
        ...
        switch (player.playerPermissionLevel) {
          case import_server9.PlayerPermissionLevel.Visitor:
            return this.Any;
          case import_server9.PlayerPermissionLevel.Member:
            return this.Member;
          case import_server9.PlayerPermissionLevel.Operator:
            return this.OP;
          case import_server9.PlayerPermissionLevel.Custom:
            return this.Admin;
          default:
            return this.Member;
        }
...
// scripts/entry.ts
var import_server35 = require("@minecraft/server");
init_Money();
init_Permission();
init_Command();
init_ConfigManager();
...
// scripts/doge/QA.ts
var import_server10 = require("@minecraft/server");
init_Tools();
init_ConfigManager();
init_Money();
init_DebugLog();
var QAManager = class _QAManager {
  ...
  start() {
    ...
    this.chatSub = import_server10.world.beforeEvents.chatSend.subscribe((event) => {
    ...
    this.timeoutId = import_server10.system.runTimeout(() => {
      this.nextQuestion();
  ...
  nextQuestion() {
    ...
    if (questions.length === 0) {
      ...
      this.timeoutId = import_server10.system.runTimeout(() => this.nextQuestion(), 20 * 60);
      return;
    }
    let questionList = [];
    let totalWeight = 0;
...
// scripts/doge/AFK.ts
var import_server11 = require("@minecraft/server");
init_ConfigManager();
init_Command();
init_Permission();
init_DebugLog();
...
function setAFK(player) {
  ...
  import_server11.world.sendMessage(`\xA77* ${player.nameTag} is now AFK. *`);
...
function startAFKScan() {
  ...
  intervalId = import_server11.system.runInterval(() => {
    let count = 0;
    for (let id in playerList) {
      let player = import_server11.world.getEntity(id);
      if (player === void 0) {
        delete playerList[id];
      } else {
        if (locationMoved(playerList[id], player.location)) {
          import_server11.world.sendMessage(`\xA77* ${player.nameTag} is no longer AFK. *`);
          player.removeTag("AFK");
...
function registerEvents() {
  import_server11.world.afterEvents.playerSpawn.subscribe((event) => {
...
function init() {
  ...
  for (let player of import_server11.world.getAllPlayers()) {
    reset(player);
  }
...
var import_server12 = require("@minecraft/server");
init_ConfigManager();
init_Command();
init_Permission();
init_Tools();
...
var Clean = class _Clean {
  ...
  placeItem(itemProvider, isFirstCall = true) {
    ...
    const dimension = import_server12.world.getDimension("overworld");
    for (let mainAxis = 0; mainAxis < this.size[0]; mainAxis++) {
      for (let y = 0; y < this.size[1]; y++) {
        ...
        let inventory = block.getComponent(import_server12.BlockComponentTypes.Inventory);
  ...
  startCleanInterval() {
    ...
    if (this.intervalId) {
      import_server12.system.clearRun(this.intervalId);
      this.intervalId = void 0;
    }
    this.intervalId = import_server12.system.runInterval(() => {
      ...
      if (entities.length > this.itemMax) {
        ...
        import_server12.system.runTimeout(() => {
          ...
          import_server12.system.runTimeout(() => {
            import_server12.world.sendMessage({ rawtext: [{ text: "\xA7a* \u5DF2\u6E05\u7406\u6389\u843D\u7269 *" }] });
          }, 5);
        }, 100);
      }
    }, this.timeout * 20);
   ...
   */
  getAllItemEntities() {
    let itemEntities = import_server12.world.getDimension("overworld").getEntities({ type: "item" });
    itemEntities.push(...import_server12.world.getDimension("nether").getEntities({ type: "item" }));
    itemEntities.push(...import_server12.world.getDimension("the_end").getEntities({ type: "item" }));
    return itemEntities;
  }
  getTimeStr() {
...
// scripts/doge/TPS.ts
var import_server13 = require("@minecraft/server");
init_Permission();
init_Command();
init_Tools();
init_DebugLog();
var TPS = class _TPS {
  ...
  static startRecord() {
    this.recordRunId = import_server13.system.runInterval(() => {
    ...
    Command.register(
      ...
      (player) => {
        ...
        if (player) {
          ...
          import_server13.world.sendMessage(msg);
        }
      },
      "\u67E5\u770B\u670D\u52A1\u5668 TPS",
      "tps"
...
var import_server14 = require("@minecraft/server");
init_Permission();
init_Command();
init_Tools();
init_HttpDB();
...
var OnlineTime = class _OnlineTime {
  ...
  registerCommandsAndPermissions() {
    ...
    Command.register(
      ...
      async (player) => {
        if (!player) {
          import_server14.world.sendMessage("\xA7c\u8BE5\u6307\u4EE4\u5FC5\u987B\u7531\u73A9\u5BB6\u6267\u884C\u3002");
          return;
        }
...
`,
  ...
  registerEvents() {
    ...
    this.playerLeaveSub = import_server14.world.afterEvents.playerSpawn.subscribe((event) => {
  ...
  init() {
    debug.i("ONLINE", "init");
    this.startTick();
    this.flushRunId = import_server14.system.runInterval(() => this.flushAll(), FLUSH_INTERVAL_TICKS);
  }
  formatTime(seconds) {
  ...
  tickSecond() {
    ...
    for (const player of import_server14.world.getAllPlayers()) {
  ...
  startTick() {
    this.tickRunId = import_server14.system.runInterval(() => {
      this.tickSecond();
    }, 20);
  }
  stop() {
...
var ChatSoundsHelper = class _ChatSoundsHelper {
  ...
  registerEvent() {
    if (this.chatSub) return;
    this.chatSub = import_server15.world.beforeEvents.chatSend.subscribe((event) => {
...
var import_server16 = require("@minecraft/server");
init_HttpDB();
...
var MonitorReporter = class {
  static init() {
    ...
    this.runId = import_server16.system.runInterval(() => {
      this.report();
    }, REPORT_INTERVAL);
  }
  static stop() {
  ...
  static async report() {
    try {
      ...
      for (const dim of DIMENSIONS) {
        try {
          entities[dim] = import_server16.world.getDimension(dim).getEntities().length;
        } catch (e) {
          entities[dim] = 0;
        }
      }
      await HttpDB.post("/api/sfmc/monitor/metrics", { tps, entities });
      const players = import_server16.world.getAllPlayers();
...
var import_server17 = require("@minecraft/server");
var SpawnProtect = class _SpawnProtect {
  ...
  static registerEvents() {
    import_server17.world.afterEvents.playerSpawn.subscribe((event) => {
...
var import_server18 = require("@minecraft/server");
var import_server_ui = require("@minecraft/server-ui");
init_Tools();
function obsStr(v = "") {
  return new import_server_ui.ObservableString(v, { clientWritable: true });
}
function obsNum(v = 0) {
  return new import_server_ui.ObservableNumber(v, { clientWritable: true });
}
function obsBool(v = false) {
  return new import_server_ui.ObservableBoolean(v, { clientWritable: true });
}
var MenuNavigator = class {
  constructor(player) {
    ...
    this.form = null;
    this.titleObs = new import_server_ui.ObservableString("");
    this.backVis = new import_server_ui.ObservableBoolean(false);
    this.state = {};
    this._confirmIdx = 0;
  ...
  section(id, title, build) {
    ...
    this.sectionVis.set(id, new import_server_ui.ObservableBoolean(false));
    return this;
  }
  ...
  async confirmMessage(title, body, confirm = "\u786E\u8BA4", cancel = "\u53D6\u6D88") {
    if (this.form?.isShowing()) this.form.close();
    const box = new import_server_ui.MessageBox(this.player, title);
    box.body(body).button1(confirm).button2(cancel);
    for (let i = 0; i < 20; i++) {
      try {
        const result = await box.show();
        if (result.closeReason === import_server_ui.DataDrivenScreenClosedReason.UserBusy) {
  ...
  async buildAndShow(token = this.sessionToken) {
    ...
    if (this.form?.isShowing()) this.form.close();
    this.form = new import_server_ui.CustomForm(this.player, this.titleObs);
    ...
    const startTick = import_server18.system.currentTick;
    let notified = false;
    while (true) {
...
var import_server19 = require("@minecraft/server");
init_HttpDB();
init_Money();
var EconomyReport = class {
  static {
    this.runId = null;
  }
  static start() {
    ...
    this.runId = import_server19.system.runTimeout(
      () => {
        this.publish();
        this.runId = import_server19.system.runInterval(() => this.publish(), 30 * 86400 * 20);
      },
      Math.ceil(delay / 50)
    );
  }
  static stop() {
    if (this.runId !== null) {
      import_server19.system.clearRun(this.runId);
      this.runId = null;
    }
  }
...
// scripts/area/Fly.ts
var import_server20 = require("@minecraft/server");
init_ConfigManager();
init_Tools();
init_Permission();
init_Tools();
...
function registerEvents2() {
  ...
  import_server20.world.afterEvents.playerSpawn.subscribe((event) => {
    if (event.initialSpawn) playerJoinEvent(event.player);
  });
}
function playerJoinEvent(player) {
  import_server20.system.runTimeout(() => {
...
function startScan2() {
  ...
  scanRunId2 = import_server20.system.runInterval(() => {
    for (let player of import_server20.world.getPlayers({ gameMode: import_server20.GameMode.Survival })) {
...
var import_server21 = require("@minecraft/server");
init_ConfigManager();
init_Tools();
init_Permission();
init_CreativeArea();
...
var SurvivalArea = class _SurvivalArea {
  ...
  registerEvents() {
    if (this.subscriptions.length > 0) return;
    this.subscriptions.push(
      import_server21.world.afterEvents.playerSpawn.subscribe((event) => {
        if (!event.initialSpawn) return;
        if (!CreativeArea.enable) return;
        if (!this.enable) return;
        const player = event.player;
        const mode = player.getGameMode();
        if (mode === import_server21.GameMode.Survival || mode === import_server21.GameMode.Adventure) return;
        import_server21.system.runTimeout(() => {
    ...
    this.subscriptions.push(
      import_server21.world.afterEvents.playerDimensionChange.subscribe((event) => {
        if (!CreativeArea.enable) return;
        if (!this.enable) return;
        const player = event.player;
        const mode = player.getGameMode();
...
var import_server22 = require("@minecraft/server");
init_ConfigManager();
init_Tools();
var InventorySwitcher = class _InventorySwitcher {
  ...
  registerEvents() {
    ...
    this.gameModeSub = import_server22.world.afterEvents.playerGameModeChange.subscribe((event) => {
      const player = event.player;
  ...
  getChestIndex(playerId2, forCreative) {
    ...
    if (base === void 0) {
      let nextIdx = import_server22.world.getDynamicProperty("hpbe:invswitcher_next");
      ...
      import_server22.world.setDynamicProperty("hpbe:invswitcher_next", base + 2);
    }
  ...
  saveToChest(player, forCreative) {
    const cfg = ConfigManager.getGrid("inventory_chest");
    if (!cfg) return;
    const dim = import_server22.world.getDimension("minecraft:overworld");
...
${time}`
    ...
    if (!block) return;
    const invComp = block.getComponent(import_server22.BlockComponentTypes.Inventory);
    ...
    if (eq) {
      ...
      const offhand = eq.getEquipment(import_server22.EquipmentSlot.Offhand);
      if (offhand) {
        eq.setEquipment(import_server22.EquipmentSlot.Offhand, void 0);
  ...
  restoreFromChest(player, forCreative) {
    ...
    const eq = player.getComponent("equippable");
    if (eq) {
      eq.setEquipment(import_server22.EquipmentSlot.Head, void 0);
      eq.setEquipment(import_server22.EquipmentSlot.Chest, void 0);
      eq.setEquipment(import_server22.EquipmentSlot.Legs, void 0);
      eq.setEquipment(import_server22.EquipmentSlot.Feet, void 0);
      eq.setEquipment(import_server22.EquipmentSlot.Offhand, void 0);
    }
    if (playerInv?.container) {
      for (let i = 0; i < 36; i++) {
    ...
    if (eq) {
      ...
      if (offhand) {
        ...
        eq.setEquipment(import_server22.EquipmentSlot.Offhand, offhand);
      }
    }
  }
};
...
var import_server25 = require("@minecraft/server");
init_Command();
init_Permission();
...
var CONFIG_REFRESH_MS = 5 * 60 * 1e3;
var _configLastFetchedAt = 0;
...
var Database = class _Database {
  ...
  static {
    this._chunkIndex = /* @__PURE__ */ new Map();
  }
  static {
    this._config = null;
  }
...
var import_server23 = require("@minecraft/server");
init_Tools();
init_Money();
...
init_LandApi();
init_DebugLog();
var ROLES = ["builder", "container", "visitor", "redstone", "entity", "admin"];
var ROLE_NAMES = {
  owner: "\u6240\u6709\u8005",
  admin: "\u7BA1\u7406\u5458",
  builder: "\u5EFA\u9020\u8005",
...
${LandCore.getDimensionName(land.dimid)} \xB7 ${info.square} \u683C \xB7 ${(land.members || []).length} \u540D\u6210\u5458`,
  ...
  async transferLand(land, status) {
    const target = import_server23.world.getPlayers().find((p) => p.id !== this.player.id);
    if (!target) {
...
var import_server24 = require("@minecraft/server");
init_HttpDB();
init_DebugLog();
var LandTax = class {
  static {
  ...
  static {
    this.CHECK_INTERVAL = 7200;
  }
  static start() {
    debug.i("LAND", "LandTax.start");
    if (this.intervalId !== null) return;
    this.intervalId = import_server24.system.runInterval(() => {
  ...
  static stop() {
    ...
    if (this.intervalId !== null) {
      import_server24.system.clearRun(this.intervalId);
      this.intervalId = null;
    }
  }
...
var LandSystem = class {
  ...
  static init() {
    ...
    LandTax.start();
    this.refreshRunId = import_server25.system.runInterval(() => {
  ...
  static cleanup() {
    ...
    LandTax.stop();
    if (this.refreshRunId !== void 0) import_server25.system.clearRun(this.refreshRunId);
...
var import_server26 = require("@minecraft/server");
init_Permission();
init_Tools();
init_DebugLog();
...
var LandEvents = class {
  ...
  static {
    this.lastLandByPlayer = /* @__PURE__ */ new Map();
  }
  static {
    this.metrics = { count: 0, totalMs: 0, slowestMs: 0 };
  }
  /** 注册事件（由 entry.ts 统一调用） */
  static registerEvents() {
    ...
    this.scanRunId = import_server26.system.runInterval(() => this.scanPlayerBoundaries(), 20);
    import_server26.world.afterEvents.playerLeave.subscribe((event) => {
    ...
    this.subscribe(import_server26.world.beforeEvents.playerInteractWithEntity, (ev) => {
      const pos = {
    ...
    this.subscribe(import_server26.world.beforeEvents.entityHurt, (ev) => {
      const source = ev.damageSource.damagingEntity;
      if (!(source instanceof import_server26.Player)) return;
    ...
    this.subscribe(import_server26.world.beforeEvents.entityItemPickup, (ev) => {
      if (!(ev.entity instanceof import_server26.Player)) return;
      const pos = {
  ...
  static scanPlayerBoundaries() {
    for (const player of import_server26.world.getPlayers()) {
      const pos = {
        x: Math.floor(player.location.x),
        y: Math.floor(player.location.y),
        z: Math.floor(player.location.z)
  ...
  static cleanup() {
    debug.i("LAND", "cleanup");
    if (this.scanRunId !== void 0) import_server26.system.clearRun(this.scanRunId);
...
var import_server27 = require("@minecraft/server");
init_Money();
init_Command();
init_Tools();
init_DebugLog();
...
var import_server30 = require("@minecraft/server");
init_Tools();
init_Money();

// scripts/gui/ChatGUI.ts
var import_server28 = require("@minecraft/server");
init_Tools();
init_Permission();
init_DogeChat();
init_Money();
...
var ChatGUI = class _ChatGUI {
  ...
  static async sendTeleportInvite(player) {
    ...
    if (channel.type === "private") {
      ...
      const target = import_server28.world.getPlayers().find((p) => p.id === otherid);
      if (!target) {
    ...
    const online = import_server28.world.getPlayers().filter((p) => p.id !== player.id);
    if (online.length === 0) {
...
var import_server29 = require("@minecraft/server");
init_Money();
init_Tools();
init_api();
init_DebugLog();
var CoopCore = class {
  ...
  static async sendToMembers(cid, text) {
    ...
    for (const member of data.members || []) {
      for (const p of import_server29.world.getPlayers())
  ...
  showMainMenu(player) {
    ...
    nav.section("transfer", "\u8F6C\u8D26", (page) => {
      ...
      page.button("\u786E\u8BA4\u8F6C\u8D26", async () => {
        ...
        import_server30.system.runTimeout(() => nav.rebuild("economy"), 40);
      });
    });
    nav.start("main");
  }
...
var import_server31 = require("@minecraft/server");
init_Command();
init_Tools();
init_DogeChat();
init_HttpDB();
...
var ChatSystem = class _ChatSystem {
  ...
  static registerEvents() {
    _ChatSystem.chatSendSub = import_server31.world.beforeEvents.chatSend.subscribe(async (event) => {
    ...
    _ChatSystem.playerJoinSub = import_server31.world.afterEvents.playerJoin.subscribe((event) => {
      const player = import_server31.world.getEntity(event.playerId);
      import_server31.system.run(async () => {
...
var import_server32 = require("@minecraft/server");
init_api();
init_Command();
init_Permission();
init_Tools();
...
function ScoreboardsBackup() {
  ...
  let entries = [];
  import_server32.world.scoreboard.getObjectives().forEach((obj, index) => {
  ...
  static registerCommands() {
    ...
    Command.register(
      ...
      async (player) => {
        ...
        else import_server32.world.sendMessage(message);
      },
      "\u4ECE\u6570\u636E\u5E93\u6062\u590D\u8BA1\u5206\u677F",
      "scoreboardSync"
    );
  ...
  static async load() {
    ...
    try {
      ...
      for (const [objId, objEntries] of groups) {
        let objective = import_server32.world.scoreboard.getObjective(objId);
        if (!objective) {
          try {
            objective = import_server32.world.scoreboard.addObjective(objId, objEntries[0].objective_display || objId);
          } catch (err) {
        ...
        for (const e of objEntries) {
          try {
            if (e.participant_type === "Player" && e.id) {
              const player = [...import_server32.world.getPlayers()].find((p) => p.id === e.id);
...
var import_server33 = require("@minecraft/server");
init_HttpDB();
init_DebugLog();
...
function enqueue(entry) {
  queue.push(entry);
  if (!flushTimer) {
    flushTimer = import_server33.system.runTimeout(flush, FLUSH_INTERVAL / 50);
  }
}
...
function subscribe() {
  ...
  const AE = import_server33.world.afterEvents;
...
var ActivityLog = class {
  ...
  static init() {
    ...
    flushIntervalId = import_server33.system.runInterval(flush, FLUSH_INTERVAL / 50);
    cleanupStartTimeoutId = import_server33.system.runTimeout(() => {
      cleanupStartTimeoutId = void 0;
      doCleanup();
      cleanupIntervalId = import_server33.system.runInterval(doCleanup, CLEANUP_INTERVAL / 50);
    }, 72e3 / 50);
  }
};

// scripts/data/World.ts
var import_server34 = require("@minecraft/server");
init_Tools();
init_api();
init_DebugLog();
function serializeGameRules() {
  const g = import_server34.world.gameRules;
  const rules = {};
  const props = [
    "commandBlockOutput",
    "doDayLightCycle",
...
async function getWorldData() {
  ...
  const data = {
    allowCheats: import_server34.world.allowCheats,
    gameRules: serializeGameRules(),
    seed: import_server34.world.seed,
    defaultSpawnLocation: JSON.stringify(import_server34.world.getDefaultSpawnLocation()),
    difficulty: import_server34.world.getDifficulty(),
    day: import_server34.world.getDay(),
    tickingAreasCount: import_server34.world.tickingAreaManager.chunkCount,
    absoluteTime: import_server34.world.getAbsoluteTime(),
    structuresFromAddon: import_server34.world.structureManager.getPackStructureIds().toString(),
...
ModuleRegistry.register({
  ...
  lifecycle: {
    ...
    registerEvents: () => {
      import_server35.world.beforeEvents.chatSend.subscribe((event) => {
...
ModuleRegistry.register({
  ...
  lifecycle: {
    registerEvents: () => {
      import_server35.world.afterEvents.playerSpawn.subscribe((event) => {
        if (!guardEvent()) return;
        if (event.initialSpawn) {
      ...
      import_server35.world.afterEvents.playerLeave.subscribe(async (event) => {
        if (!guardEvent()) return;
        const player = import_server35.world.getEntity(event.playerId);
        if (player) {
          try {
...
var AddOnInit = class {
  ...
  static registerEvents() {
    import_server35.system.beforeEvents.startup.subscribe(async () => {
      import_server35.system.run(async () => {
    ...
    import_server35.world.afterEvents.worldLoad.subscribe(() => {
      if (!guardEvent()) return;
      ModuleRegistry.bootAfterWorldLoad();
      syncWorldData();
    });
    import_server35.system.beforeEvents.shutdown.subscribe(() => {
      if (!guardEvent()) return;
