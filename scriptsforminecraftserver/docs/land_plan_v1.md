...
1. 校验 actorId == owner_player_id
1. 校验 version，防止旧 GUI 覆盖新状态
1. 计算退款金额
1. 扣除/标记土地状态
1. 写入经济流水
1. 写入审计日志
...
1. 从 Database._registry 删除土地。
1. 重建 Owner 索引。
1. 重建 chunk 索引。
1. 清除当前 GUI 的 selectedLandId。
1. 使用 nav.replace("home") 返回土地中心。
...
9. 启动期间 fail-closed
   验证步骤：
10. 启动 BDS，但让 db-server 暂时不可用。
11. 玩家尝试放置和破坏方块。
12. 玩家尝试打开容器、使用门、按钮和红石设备。
13. 确认土地缓存尚未取得权威快照时，土地保护不会默认放行。
14. db-server 恢复后，等待土地快照加载。
15. 确认正常土地权限恢复。
    预期：
...
7. 验证并发删除不会重复退款。
   阶段 2：转让空玩家交互
8. 修改 LandGUI.transferLand()。
9. 无目标玩家时使用 MessageBox。
10. 转让请求改为 runTask()。
11. 增加转让错误码和版本冲突处理。
12. 验证目标玩家在确认前离线时不会产生错误状态。
    阶段 3：统一土地 API 结果
13. 定义 LandErrorCode。
14. 定义 LandApiResult<T>。
15. 改造删除和转让 API。
16. 再逐步改造成员、邀请、创建和更新 API。
17. 统一 GUI 错误显示和网络异常处理。
    阶段 4：保护和生命周期验收
18. 修复 LandPolicy 中基于标签的绕过。
19. 验证启动 fail-closed。
20. 验证实体、拾取、爆炸和自动化装置。
21. 验证模块 disable/re-enable。
22. 记录无法直接拦截的 SAPI 事件并确定补偿策略。
    阶段 5：自动化回归测试
    优先增加以下测试：
