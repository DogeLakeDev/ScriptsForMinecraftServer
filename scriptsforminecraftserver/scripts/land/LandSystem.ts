...
export class LandSystem {
  static init() {
    ...
    Command.register("land", "land.use", (player: Player | undefined) => {
      if (!player) return "§c该指令只能由玩家执行。";
      LandGUI.showMainMenu(player);
    }, "土地管理");

    Command.register("land cancel", "land.use", (player: Player | undefined) => {
      if (!player) return "§c该指令只能由玩家执行。";
    ...
    Command.register("pos1", "land.use", (player: Player | undefined) => {
      if (!player) return "§c该指令只能由玩家执行";
      handlePosCommand(player, 1);
    }, "设置土地第一点");

    Command.register("pos2", "land.use", (player: Player | undefined) => {
      if (!player) return "§c该指令只能由玩家执行";
