...
const CONTAINER_BLOCKS = new Set([
  ...
  "minecraft:barrel", "minecraft:ender_chest", "minecraft:hopper", "minecraft:dispenser", "minecraft:dropper",
  // 潜影盒用正则匹配
]);
...
function checkLandPermission(player: Player, pos: LandPos, dimid: number, capability: Parameters<typeof canUseAt>[3]): boolean {
  ...
  // 管理员/OP 跳过检查
  if (Permission.getPermission(player) >= Permission.OP) { recordMetric(Date.now() - started); return true; }
...
export class LandEvents {
  ...
  static registerEvents() {
    ...
    this.subscribe(world.beforeEvents.entityHurt, (ev) => {
      ...
      const pos = { x: Math.floor(target.location.x), y: Math.floor(target.location.y), z: Math.floor(target.location.z) };
      const dimid = target.dimension.id === "minecraft:overworld" ? 0 : target.dimension.id === "minecraft:nether" ? 1 : 2;
      if (!checkLandPermission(source, pos, dimid, "attack_entity")) { ev.cancel = true; Msg.error("你没有权限攻击此土地内的实体！", source); }
    });

    this.subscribe(world.beforeEvents.entityItemPickup, (ev) => {
      if (!(ev.entity instanceof Player)) return;
      const pos = { x: Math.floor(ev.item.location.x), y: Math.floor(ev.item.location.y), z: Math.floor(ev.item.location.z) };
      const dimid = ev.item.dimension.id === "minecraft:overworld" ? 0 : ev.item.dimension.id === "minecraft:nether" ? 1 : 2;
      if (!checkLandPermission(ev.entity, pos, dimid, "pickup_item")) { ev.cancel = true; Msg.error("你没有权限拾取此土地内的物品！", ev.entity); }
    });
  ...
  private static subscribe<T>(signal: { subscribe(callback: (event: T) => void): (event: T) => void; unsubscribe(callback: (event: T) => void): void }, callback: (event: T) => void): void {
  ...
  static cleanup() {
    ...
    for (const s of this.subscriptions) {
      try { s.signal.unsubscribe(s.callback); } catch {}
    }
    this.subscriptions = [];
