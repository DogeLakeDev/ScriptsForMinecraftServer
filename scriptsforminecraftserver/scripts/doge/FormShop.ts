...
export class FormShop {
  ...
  static showForm(player: Player, formData: any, callList: number[], isSell: boolean) {
    ...
    if (formData["type"] === "group" || formData["type"] === "root") {
      ...
      form.toggle("取消并返回上一页", { defaultValue: false } as any);
      form.submitButton("确 认");

      form.show(player).then((res) => {
        if (res.canceled) return;
        if ((res.formValues as any[])[1] === true) {
          if (callList.length === 1) {
            this.showFormMain(player);
          }
          else {
          ...
          let amount = parseInt((res.formValues as any[])[0] as string);
   ...
   * 返回上一页的数据，必定是组/根
   */
  static getFormDataByCallList(callList: number[]) {
   ...
   * 进行交易
   */
  static trade(player: Player, data: any, amount: number, isSell: boolean, itemName: any) {
    // 创建模板物品
    ...
    system.runTimeout(() => {
      let tempContainer = (tempEntity.getComponent("inventory") as any).container;
      let targetItem = tempContainer.getItem(0);

      if (isSell) {// 卖出
        let remainingAmount = amount;
        let playerContainer = (player.getComponent("inventory") as any).container;
        ...
        if (successAmount > 0) {
          ...
          system.runTimeout(() => {
            let rawtext: any[] = [{ "text": `[商店] 卖出 §e§l${successAmount}§r 个 §e` }];
        ...
        if (successAmount > 0) {
          ...
          system.runTimeout(() => {
            let rawtext: any[] = [{ "text": `[商店] 买入 §e§l${successAmount}§r 个 §e` }];
            rawtext.push(itemName[0]);
            rawtext.push(itemName[1])
  ...
  static registerScriptCommand() {
    Command.register("shop", Permission.Any, this.showFormMain as unknown as (player: Player | undefined) => any, "打开商店");
  }
}
