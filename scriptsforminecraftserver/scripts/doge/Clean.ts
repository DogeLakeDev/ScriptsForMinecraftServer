 ...
 */
export class Clean {
  static _instance: Clean | undefined = undefined;
  static getInstance() {
    if (!Clean._instance) {
   ...
   */
  placeItem(itemProvider: () => ItemStack | undefined, isFirstCall: boolean = true) {
    // 确定面的增长方向
    let base = [1, 0];
    ...
    for (let mainAxis = 0; mainAxis < this.size[0]; mainAxis++) {
      for (let y = 0; y < this.size[1]; y++) {
        ...
        // 获取主箱子的容器（可以获取到连体的容器）
        let inventory = block!.getComponent(BlockComponentTypes.Inventory) as any;
        ...
        (sign!.getComponent(BlockComponentTypes.Sign) as any).setText(this.getTimeStr());
      }
    }
  ...
  startClean(entities?: Entity[]) {
    ...
    this.placeItem(() => {
      while (itemEntities.length > 0) {
        ...
        let stack = (itemEntity.getComponent("minecraft:item") as any).itemStack;
        if (!stack) {
          continue;
        }
        // 在直接清除列表中的物品实体 直接清除
...
function registerCommand() {
  Command.register("clean", (Permission as any).OP, () => {
