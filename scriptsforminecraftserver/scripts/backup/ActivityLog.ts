...
function dimId(entityOrBlock: Entity | Block): string {
  try { return entityOrBlock.dimension?.id?.replace('minecraft:', '') || ''; }
  catch { return ''; }
}
...
function playerId(player: Player): string {
  try { return player.id || ''; } catch { return ''; }
}
...
function getTargetPlayerId(entity: Entity): string {
  ...
  try { return (entity as Player).id || ''; } catch { return ''; }
}

function getTargetPlayerName(entity: Entity): string {
  if (entity.typeId !== 'minecraft:player') return entity.typeId;
  try { return (entity as Player).name || entity.typeId; } catch { return entity.typeId; }
}
...
function subscribe() {
  ...
  world.afterEvents.entityHitEntity.subscribe((event) => {
    ...
    if (attacker.typeId === 'minecraft:player') {
      enqueue(playerEntry(attacker as Player, 'entity.hit', {
  ...
  world.afterEvents.entityTamed.subscribe((event) => {
    ...
    const [tx, ty, tz] = loc(target.location);
    enqueue(playerEntry(tamer as Player, 'entity.tame', {
      targetType: 'entity',
  ...
  world.afterEvents.entityItemDrop.subscribe((event) => {
    ...
    if (e.typeId === 'minecraft:player') {
      enqueue(playerEntry(e as Player, 'item.drop', {
        detail: {
          items: event.items.map(item =>
            typeof item === 'string' ? item : (item as any)?.typeId
          ).filter(Boolean),
        },
      }));
    } else {
  ...
  world.afterEvents.entityItemPickup.subscribe((event) => {
    ...
    const [ex, ey, ez] = loc(e.location);
    if (e.typeId === 'minecraft:player') {
      enqueue(playerEntry(e as Player, 'item.pickup', {
        detail: {
  ...
  world.afterEvents.blockContainerOpened.subscribe((event) => {
    ...
    const [bx, by, bz] = loc(event.block.location);
    enqueue(playerEntry(source as Player, 'container.open', {
      targetType: 'block',
      targetName: event.block.typeId,
      targetX: bx, targetY: by, targetZ: bz,
    }));
  ...
  world.afterEvents.blockContainerClosed.subscribe((event) => {
    ...
    if (!source || source.typeId !== 'minecraft:player') return;
    const [bx, by, bz] = loc(event.block.location);
    enqueue(playerEntry(source as Player, 'container.close', {
      targetType: 'block',
      targetName: event.block.typeId,
      targetX: bx, targetY: by, targetZ: bz,
    }));
