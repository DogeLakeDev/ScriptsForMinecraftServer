...
import { ProjectionData, ColorPreset, COLOR_PRESETS } from "../data/HoloPrint";
...
export class HoloGUI {
  static registerCommand() {
    Command.register(
      ...
      (player: Player | undefined) => {
        if (player) HoloGUI.showMainMenu(player);
      },
      "全息投影"
    );
    Command.register(
  ...
  static async showUploadConfig(player: Player): Promise<void> {
  ...
  static async showProjectionList(
    ...
    const form = new CustomForm(player, "加载投影").button("§l=== 我的投影 ===", () => {
  ...
  static async showMaterialList(player: Player, materials: { name: string; count: number }[]): Promise<void> {
    ...
    const form = new CustomForm(player, "物品清单").label(`共 §e${sorted.length}§r 种材料`);
  ...
  static async showColorPicker(player: Player, title: string): Promise<string | null> {
    let result: string | null = null;
    const form = new CustomForm(player, title).label("选择一个颜色预设：");
  ...
  private static async showMoveInput(player: Player, projection: ProjectionData): Promise<void> {
    const s = projection.settings;
  ...
  private static async showLayerModePicker(player: Player, projection: ProjectionData): Promise<void> {
