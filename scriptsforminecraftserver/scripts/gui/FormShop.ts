...
let _shopTree: any[] | null = null;

function getShopTree(): any[] {
  if (_shopTree) return _shopTree;
  const categories = ConfigManager.getShopCategories();
  const items = ConfigManager.getShopItems();

  const itemByCat: Record<number, any> = {};
  for (const item of items) itemByCat[item.category_id] = item;
  ...
  for (const cat of categories) {
    ...
    if (cat.parent_id == null) {
      if (cat.type === "root" && cat.name === "卖出") node.sell = true;
      roots.push(node);
    } else {
      const parent = nodeMap[cat.parent_id];
      if (parent) parent.data.push(node);
    }
  }
  _shopTree = roots;
  return roots;
...
function getFormData(callList: number[]): any {
  const tree = getShopTree();
  return callList.slice(1).reduce((node: any, idx: number) => node?.data?.[idx], tree[0]);
}

function getParentList(callList: number[]): number[] {
  return callList.length > 1 ? callList.slice(0, -1) : [];
}

function getNodeName(node: any): string {
  if (!node) return "";
  if (node.type === "exam") return `${node.name}\n§a$${node.data?.money ?? ""}`;
  return node.name ?? "";
}
...
export class FormShop {
  static showFormMain(player: Player) {
    ...
    nav.section("list", "商店", (page) => {
      const callList: number[] = nav.state.callList ?? [0];
    ...
    nav.section("buy", "购买", (page) => {
      ...
      const examData = nav.state.examData as any;
      const isSell = nav.state.isSell as boolean;
      if (!examData) {
        page.label("商品数据丢失。");
        return;
      }
