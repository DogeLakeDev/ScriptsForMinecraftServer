...
export class CoopGUI {
  ...
  confirmPop(title: string, text: string, onConfirm: () => void) {
  ...
  shopMgr(cid: string, step: number, gid?: string) {
    ...
    switch (step) {
      ...
      case 1: {
        ...
        form.button("§l返回", () => this.coopInfoPanel(cid, "menu")).closeButton();
      ...
      case 6: {
        ...
        const form = new CustomForm(this.player, "商店管理后台").label(ListFormInfo(["回收物品管理"]));
        for (const g of goods2) {
          const goodItem = g;
          form.button(_fmtGoodBt(g.name, unit, g.money, g.sv, g.num, false), () => this.shopMgr(cid, 7, goodItem.id));
        }
        form.button("§l返回", () => this.shopMgr(cid, 1)).closeButton();
        Gui.showForm(this.player, form, "商店管理后台").catch(() => {});
        break;
      }
