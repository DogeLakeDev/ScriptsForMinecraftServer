...
import { Database as DB, ShopItem } from "../coop/Database";
...
export class CoopGUI {
  ...
  errorPop(text: string) { Msg.error(text, this.player); }
  tipsPop(text: string) { Msg.tips(text, this.player); }
  infoPop(text: string) { Msg.info(text, this.player); }
  confirmPop(title: string, text: string, onConfirm: () => void) { Gui.confirm(this.player, title, text, onConfirm); }
  ...
  private noCoopPanel() {
    Gui.simpleForm()
      ...
      .button("合作社排行榜")
      .button("插件更新日志")
      .show(this.player).then((res: any) => {
        if (res.canceled) return;
        switch (res.selection) {
          case 0: this.joinByCid(); break;
          case 1: this.coopList(); break;
          case 2: this.createCoop(); break;
          case 3: this.rank(1); break;
          case 4: this.log(); break;
        }
      }).catch(() => {});
  }
  ...
  private joinByCid() {
    Gui.modalForm()
      ...
      .show(this.player).then((res: any) => {
        if (res.canceled) { this.mainPanel(); return; }
        const cid = (res.formValues![0] as string)?.trim();
        if (!cid) { this.errorPop("请填写CID"); return; }
        const data = DB.getCoopByCid(cid);
        if (!data) { this.errorPop("请检查CID是否正确"); return; }
  ...
  private coopList() {
    ...
    if (all.length === 0) { this.errorPop("还没有任何合作社"); return; }
    const form = Gui.simpleForm().title("合作社列表");
    for (const c of all) form.button(c.name);
    form.button("§l返回");
    form.show(this.player).then((res: any) => {
      if (res.canceled) { this.mainPanel(); return; }
      if (res.selection === all.length) { this.mainPanel(); return; }
  ...
  private createCoop() {
    Gui.modalForm()
      ...
      .show(this.player).then((res: any) => {
        ...
        const cid = vals[1] as string;
        if (!name || !cid) { this.errorPop("请填写完整信息"); return; }
  ...
  coopInfoPanel(cid: string, returnMode: string) {
    const text = CoopCore.getInfo(cid);
    if (returnMode === "info") { this.infoPop(text); return; }
    if (returnMode === "join") {
    ...
    form.show(this.player).then((res: any) => {
      if (res.canceled) return;
      switch (res.selection) {
        case 0: this.shopMgr(cid, 1); break;
        case 1: this.bankPanel(cid); break;
        case 2: this.infoPop(CoopCore.getMemberList(cid).join(", ")); break;
        case 3: this.coopList(); break;
        case 4: this.rank(1); break;
        case 5: this.exitConfirm(cid); break;
        case 6: this.log(); break;
        case 7: this.adminPanel(cid); break;
      }
    }).catch(() => {});
  }

  private exitConfirm(cid: string) {
    const isOp = CoopCore.isOp(this.player.name, cid);
    this.confirmPop("合作社 - 确认",
  ...
  private adminPanel(cid: string) {
    Gui.simpleForm().title("合作社 - 管理面板")
      ...
      .button("§l返回")
      .show(this.player).then((res: any) => {
        if (res.canceled) { this.coopInfoPanel(cid, "menu"); return; }
        if (res.selection === 3) { this.coopInfoPanel(cid, "menu"); return; }
        switch (res.selection) {
          case 0: this.editNotice(cid); break;
          case 1: this.talkToMembers(cid); break;
          case 2: this.addAdmin(cid); break;
        }
      }).catch(() => {});
  }

  private editNotice(cid: string) {
    Gui.modalForm().title("合作社 - 编辑公告").textField("公告内容", "")
      .show(this.player).then((res: any) => {
        if (res.canceled) { this.coopInfoPanel(cid, "menu"); return; }
        CoopCore.setNotice(cid, (res.formValues![0] as string) || "");
        this.infoPop("设置成功。");
      }).catch(() => {});
  }

  private talkToMembers(cid: string) {
  ...
  private bankPanel(cid: string) {
    ...
    Gui.modalForm().title("合作社 - 银行")
      ...
      .show(this.player).then((res: any) => {
        if (res.canceled) { this.coopInfoPanel(cid, "menu"); return; }
        this.bankControl(cid, (res.formValues![0] as number) + 1);
      }).catch(() => {});
  }

  private bankControl(cid: string, type: number) {
    const title = type === 1 ? "存入" + Money.UNIT : "取出" + Money.UNIT;
    const form = Gui.modalForm().title("合作社 - " + title)
      .textField("金额", "").textField("备注(可选)", "无");
    form.show(this.player).then((res: any) => {
      if (res.canceled) { this.coopInfoPanel(cid, "menu"); return; }
      const val = parseInt(res.formValues![0] as string);
      if (isNaN(val) || val <= 0) { this.errorPop("金额填写不正确"); return; }
      if (CoopCore.bankControl(cid, this.player, val, (res.formValues![1] as string) || "", type === 1 ? 1 : 2)) {
        if (type === 1) Msg.success("存入成功！" + Money.UNIT + "：" + val, this.player);
        else Msg.success("取出成功！" + Money.UNIT + "：" + val, this.player);
      } else { this.errorPop("金额填写不正确"); }
    }).catch(() => {});
  }
  ...
  shopMgr(cid: string, step: number, gid?: string) {
    const isOp = CoopCore.isOp(this.player.name, cid);
    ...
    switch (step) {
      ...
      case 1: {
        ...
        form.show(this.player).then((res: any) => {
          ...
          if (idx === backIdx) { this.coopInfoPanel(cid, "menu"); return; }
      ...
      case 2: {
        ...
        Gui.modalForm().title("商店管理后台").textField("gid:" + gid, "")
          ...
          .show(this.player).then((res: any) => {
            if (!res.canceled) this.shopMgr(cid, (res.formValues![1] as number) + 3, gid);
          }).catch(() => {});
        break;
      }
      ...
      case 3: {
        ...
        Gui.modalForm().title("补货").textField("当前库存：" + good.num, "")
          ...
          .show(this.player).then((res: any) => {
            if (res.canceled) { this.shopMgr(cid, 1); return; }
            const num = res.formValues![1] as number;
            if (num <= 0) { this.errorPop("请填写完整信息！"); return; }
            good.num += num;
            DB.saveGood(good);
      ...
      case 5: {
        ...
        Gui.modalForm().title("编辑商品信息")
          ...
          .show(this.player).then((res: any) => {
            if (res.canceled) { this.shopMgr(cid, 1); return; }
            const vals = res.formValues!;
            good.name = vals[0] as string;
            good.des = vals[1] as string;
            good.money = parseInt(vals[2] as string) || 0;
            const cgIdx = vals[3] as number;
            if (cgIdx > 0) {
              const idx = good.groups.findIndex((g: string) => customGroups.some((cg: any) => cg.groupid === g));
      ...
      case 6: {
        ...
        form.show(this.player).then((res: any) => {
          if (res.canceled) { this.shopMgr(cid, 1); return; }
          if (res.selection === goods2.length) { this.shopMgr(cid, 1); return; }
      ...
      // ---- step 7: 取出回收库存 ----
      case 7: {
        if (!good || good.sv <= 0) { this.errorPop("暂时没有需要取出的库存。"); break; }
        Gui.modalForm().title("取出回收库存")
          .slider("取出数量", 1, good.sv, { valueStep: 1, defaultValue: 1 })
          .show(this.player).then((res: any) => {
            if (res.canceled) { this.shopMgr(cid, 1); return; }
            const num = res.formValues![0] as number;
            good.sv -= num;
            DB.saveGood(good);
  ...
  shopAdd(cid: string, step: number, index?: number) {
    switch (step) {
      // ---- step 1: 选择物品栏和操作类型 ----
      case 1: {
        Gui.modalForm().title("上架物品")
          ...
          .show(this.player).then((res: any) => {
            if (!res.canceled) this.shopAdd(cid, (res.formValues![1] as number) + 2, res.formValues![0] as number);
          }).catch(() => {});
        break;
      }
      ...
      case 2: {
        ...
        if (!item) { this.errorPop("请确认物品栏有物品"); return; }
        const customGroups = CoopCore.getGroups(true);
        const cgNames = ["无", ...customGroups.map((g: any) => g.displayname)];
      ...
      case 4: {
        Gui.modalForm().title("添加自定义分组").textField("分组名称", "")
          .show(this.player).then((res: any) => {
            ...
            if (!name) { this.errorPop("请填写完整信息！"); return; }
