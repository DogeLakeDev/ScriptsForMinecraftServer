...
export class CreativeArea {
    ...
    registerEvents() {
        ...
        this.subscriptions.push(world.afterEvents.playerSpawn.subscribe((event) => {
            ...
            system.runTimeout(() => {
                ...
                else if (event.player.getGameMode() === GameMode.Creative ||
        ...
        this.subscriptions.push(world.afterEvents.entitySpawn.subscribe((event) => {
            if (!CreativeArea.enable)
                return;
            if (!event.entity)
                return;
        ...
        this.subscriptions.push(world.beforeEvents.playerPlaceBlock.subscribe((event) => {
            if (!CreativeArea.enable)
                return;
        ...
        this.subscriptions.push(world.beforeEvents.playerBreakBlock.subscribe((event) => {
            if (!CreativeArea.enable)
                return;
    ...
    inBufferZone(entity) {
        for (const area of ConfigManager.getAreas("creative")) {
            ...
            const inExpanded = x >= minX - this.BUFFER_ZONE &&
    ...
    restoreScores(player) {
        ...
        const scores = raw ? JSON.parse(raw) : undefined;
        if (!scores)
            return;
        const identity = player.scoreboardIdentity;
        if (!identity)
    ...
    // ==========================================
    startTick() {
        this.tickRunIds.push(system.runInterval(() => {
            if (!CreativeArea.enable)
                return;
