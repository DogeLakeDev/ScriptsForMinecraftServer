...
export class CoopGUI {
    ...
    // ── No Coop ──
    buildNoCoop(page) {
        page.label(ListFormInfo(["你没有加入任何一个合作社，请选择操作。\n\nCiallo～(\u2220\u30fb\u03c9\uff1c)\u2322\u2606"]));
    ...
    async buildCoopInfo(page) {
        ...
        page.button("插件更新日志", () => this.nav.go("log"));
        if (isOp)
            page.button("管理面板", () => this.nav.rebuild("adminPanel"));
    }
    async exitConfirm(cid) {
        const isOp = await CoopCore.isOp(this.player.name, cid);
        this.nav.confirm("合作社 - 确认", isOp ? "确认解散合作社？所有成员也会被踢出。\n请先清空银行经济、下架商品。" : "你确认退出合作社吗？", async () => {
            if (isOp) {
    ...
    async buildAddAdmin(page) {
        ...
        page.button("确认", () => {
            ...
            this.nav.confirm("合作社 - 确认", "目标玩家会获得管理面板的使用权，确认操作吗？", async () => {
    ...
    buildShopItemOps(page) {
        ...
        page.button("确认", () => {
            ...
            if (act === 0)
                this.nav.go("shopRestock");
            else if (act === 1)
                this.doDelist(cid, gid, good);
            else
                this.nav.go("shopEdit");
        });
    }
    buildShopRestock(page) {
        ...
        page.button("确认", async () => {
            ...
            this.player.runCommand('clear "' + this.player.name + '" ' + good.item_type + " " + (good.item_aux ?? 0) + " " + num);
            status.ok("补货成功。");
            this.nav.rebuild("shopMgr");
        });
    }
    doDelist(cid, gid, good) {
        this.nav.confirm("下架确认", "确认下架 " + good.name + " ？\n下架后库存将返还给你。", async () => {
            await CoopAPI.deleteShopItem(good.cid, gid);
            this.player.runCommand('give "' + this.player.name + '" ' + good.item_type + " " + good.num + " " + (good.item_aux ?? 0));
    ...
    buildShopRecycleTake(page) {
        ...
        page.button("确认", async () => {
            ...
            good.sv -= num;
            await CoopAPI.saveShopItem(good);
            this.player.runCommand('give "' + this.player.name + '" ' + good.item_type + " " + num + " " + (good.item_aux ?? 0));
            status.ok("取出成功。");
    ...
    async buildShopRecycleReview(page) {
        ...
        for (const g of goods1) {
            page.button(g.name + " " + CoopCore.getConfig().shop_setting.monetary_unit + g.money + "\n待审核", () => {
                this.nav.confirm("回收招募审核", `名称: ${g.name}\n描述: ${g.des || ""}\n价格: ${g.money}\n库存: ${g.num}\n\n确定通过审核？`, async () => {
                    g.is_true = true;
