...
export class ChatGUI {
    ...
    async buildPanel(page) {
        ...
        const displayChannels = allChannels.filter((c) => {
            if (c.type === "private")
                return false;
            if (c.type === "system")
                return c.ownerid === this.player.id;
            return true;
        });
        ...
        for (const c of displayChannels) {
            ...
            const label = obsStr(`${subscribed ? "§a☑" : "§7☐"} ${c.prefix} - ${c.name}\n§a${DogeChat.getOnlineCount(c.id)} 人在线`);
    ...
    async buildSettings(page) {
        ...
        if (isOwner && channel.type !== "public") {
            page.button("删除频道", () => {
                this.nav.confirm("删除频道", `确认删除频道 "${channel.name}" 吗？此操作不可撤销。`, () => DogeChat.deleteChannel(channel.id).then(() => status.ok(`频道 "${channel.name}" 已删除。`)), () => this.nav.rebuild("manager"));
            });
        }
    }
    ...
    async buildClaimRedPacket(page) {
        ...
        for (const p of packets) {
            page.button(`${p.senderName} 的红包 §7(${p.remainingAmount} 剩余)`, async () => {
                ...
                if (amount > 0)
                    status.ok(`领取了 ${amount} ${Money.UNIT}！`);
                else
                    status.fail("领取失败");
            });
        }
