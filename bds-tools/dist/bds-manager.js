...
function createBdsManager() {
    ...
    const sendCommand = (cmd) => {
        ...
        log_js_1.log.warn("BDS 由外部启动，无法发送命令到 stdin");
        return false;
    };
    const stop = async () => {
        const p = ensureProc();
        ...
        if (!pid || !(await isAlive(pid))) {
            log_js_1.log.info("BDS 未运行");
            clearPid();
            return;
        }
        if (p.process && p.process.pid === pid && p.process.stdin) {
            p.isManualStop = true;
            log_js_1.log.info("正在关闭 BDS...");
            try {
                p.process.stdin.write("stop\n");
            }
            catch (e) {
                log_js_1.log.warn(`发送 stop 命令失败: ${e.message}`);
            }
            await Promise.race([
                new Promise((resolve) => p.process?.once("exit", () => resolve())),
                new Promise((resolve) => setTimeout(() => {
                    log_js_1.log.warn("30s 超时，强制终止...");
                    try {
            ...
            log_js_1.log.info("BDS 由外部启动，使用 taskkill...");
            try {
                if (process.platform === "win32") {
        ...
        clearPid();
        p.process = null;
        log_js_1.log.info("BDS 已停止");
    };
    const start = async () => {
        const p = ensureProc();
        const existing = readPid();
        if (existing && (await isAlive(existing))) {
            log_js_1.log.info("BDS 已在运行中");
            return;
        }
        if (!node_fs_1.default.existsSync(p.exePath)) {
            log_js_1.log.error(`未找到 ${p.exePath}`);
        ...
        child.on("exit", (code) => {
            log_js_1.log.info(`BDS 已退出 (code: ${code})`);
            clearPid();
            p.process = null;
            if (!p.isManualStop && p.crashRestart && isMain()) {
                log_js_1.log.info(`BDS 意外退出，${p.crashDelayMs / 1000}s 后自动重启...`);
                setTimeout(() => {
                    start().catch((e) => log_js_1.log.error(`自动重启失败: ${e.message}`));
                }, p.crashDelayMs);
            }
        ...
        while (true) {
            ...
            if (!pid || !(await isAlive(pid))) {
                try {
                    ...
                    log_js_1.log.error(`自动启动失败: ${e.message}`);
                }
            }
...
if (isMain()) {
    ...
    switch (cmd) {
        case "start":
            bds.start().catch((e) => log_js_1.log.error(`启动失败: ${e.message}`));
            break;
        case "stop":
            bds.stop().catch((e) => log_js_1.log.error(`停止失败: ${e.message}`));
            break;
        case "restart":
            bds.stop()
                .then(() => bds.start())
                .catch((e) => log_js_1.log.error(`重启失败: ${e.message}`));
            break;
        case "status":
            bds.status();
            break;
        case "send":
            ...
            break;
        case "watch":
            bds.watch().catch((e) => log_js_1.log.error(`监护异常: ${e.message}`));
            break;
        default:
            console.log(`用法:
  start         启动 BDS
