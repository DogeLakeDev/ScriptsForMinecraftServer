...
async function doBackup(bdsPath, backupDir, preserve) {
    ...
    for (const item of preserve) {
        ...
        anyCopied = true;
        logger_js_1.logger.info(`已备份: ${item}`);
    }
    // 即使全部 preserve 都跳过，也让目录存在以便记录时间戳
    if (!anyCopied) {
        logger_js_1.logger.info(`没有需要备份的文件 (preserve 列表为空或全部缺失)`);
    }
...
async function restorePreserves(bdsPath, backupDir, preserve) {
    for (const item of preserve) {
        ...
        try {
            ...
            logger_js_1.logger.info(`已恢复: ${item}`);
        }
        catch (e) {
            logger_js_1.logger.warn(`恢复失败 ${item}: ${e.message}`);
        }
    }
}
...
async function runUpdate() {
    ...
    logger_js_1.logger.info(`===== 开始检查更新 (${channel}) =====`);
    // 1. 当前版本
    ...
    try {
        ...
        logger_js_1.logger.error(`获取下载详情失败: ${e.message}`);
        await (0, qqutil_js_1.sendText)(`❌ BDS 更新失败\n\n下载详情获取失败: ${e.message}`);
        return 1;
    }
    if (checkOnly) {
    ...
    logger_js_1.logger.info(`发现新版本: ${currentVer} → ${latestVer}`);
    // 4. QQ 预告
    if (qqNotify) {
    ...
    try {
        ...
        logger_js_1.logger.info(`备份完成: ${backupInfo.path} (${(backupInfo.size / 1024 / 1024).toFixed(1)} MB)`);
        if (qqNotify) {
            await (0, qqutil_js_1.sendText)(`✅ 备份完成\n\n大小: ${(backupInfo.size / 1024 / 1024).toFixed(1)} MB\n位置: ${backupInfo.path}`);
        }
    }
    catch (e) {
        logger_js_1.logger.error(`备份失败: ${e.message}`);
        await (0, qqutil_js_1.sendText)(`❌ BDS 更新失败\n\n备份失败: ${e.message}\n操作已中止`);
        return 1;
    }
    // 6. 写回滚标记 (跨进程可恢复)
    ...
    try {
        logger_js_1.logger.info("停止 BDS 服务...");
        await bds.stop();
        logger_js_1.logger.info("BDS 已停止");
    }
    catch (e) {
        logger_js_1.logger.warn(`BDS 停止异常: ${e.message}`);
    }
    // 8. 下载 (全部失败则回滚)
    ...
    try {
        logger_js_1.logger.info(`开始下载 ${latestVer}...`);
        if (qqNotify)
        ...
        for (const url of downloadUrls) {
            try {
                ...
                if (barStarted)
                    progressBar.stop();
                lastErr = e;
                logger_js_1.logger.warn(`${url} 不可用: ${lastErr.message}，尝试备用地址...`);
            }
        }
        if (!downloaded && lastErr)
            throw lastErr;
        const zipSizeMB = (node_fs_1.default.statSync(zipPath).size / 1024 / 1024).toFixed(1);
        logger_js_1.logger.info(`下载完成 (${zipSizeMB} MB)`);
        // 流式哈希校验
        if (hash.sha1 || hash.sha256) {
            const algo = hash.sha256 ? "SHA256" : "SHA1";
            logger_js_1.logger.info(`校验文件完整性 (${algo})...`);
            const ok = await (0, upstream_js_1.verifyFileHash)(zipPath, hash.sha1, hash.sha256);
            if (!ok)
                throw new Error(`${algo} 校验不通过，文件可能损坏`);
            logger_js_1.logger.info(`${algo} 校验通过`);
        }
    }
    catch (e) {
        logger_js_1.logger.error(`下载失败: ${e.message}`);
        await (0, qqutil_js_1.sendText)(`❌ BDS 更新失败\n\n下载失败: ${e.message}\nBDS 已停止，请手动恢复`);
        // 自动回滚
        (0, rollback_js_1.rollbackFromBackup)({ timestamp: Date.now(), bds_path: bdsPath, backup_dir: backupInfo.path, preserve, previous_version: currentVer });
        (0, fsx_js_1.rmSafe)(stagingDir);
        return 1;
    }
    // 9. 解压 + 部署
    try {
        logger_js_1.logger.info("解压中...");
        // 先清空 BDS 目录
        (0, fsx_js_1.emptyDirSync)(bdsPath);
        extractZipToBds(zipPath, bdsPath);
        logger_js_1.logger.info("解压完成");
    }
    catch (e) {
        logger_js_1.logger.error(`解压失败: ${e.message}`);
        await (0, qqutil_js_1.sendText)(`❌ BDS 更新失败\n\n解压失败: ${e.message}\nBDS 已停止，请手动恢复`);
    ...
    const verify = (0, rollback_js_1.verifyBdsInstall)(bdsPath, ["bedrock_server.exe", /*"bedrock_server_symbols.debug",*/ "permissions.json"]);
    if (!verify.ok) {
        logger_js_1.logger.error(`解压后的 BDS 不完整，缺失文件: ${verify.missing.join(", ")}`);
    ...
    if (newExeHash) {
        ...
        logger_js_1.logger.info(`已记录版本 ${latestVer} 的 SHA256`);
    }
    // 13. 启动 BDS
    if (autoRestart) {
        try {
            logger_js_1.logger.info("启动 BDS...");
            await bds.start();
        }
        catch (e) {
            logger_js_1.logger.error(`启动失败: ${e.message}`);
            await (0, qqutil_js_1.sendText)(`❌ BDS 更新失败\n\n启动失败: ${e.message}\n请手动启动 BDS`);
    ...
    logger_js_1.logger.info(`===== 更新完成 (${(durationMs / 1000).toFixed(1)}s) =====`);
    return 0;
}
function isMain() {
