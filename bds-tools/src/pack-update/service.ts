...
import type { PackUpdateBindingsApi } from "./bindings.js";
import type { PackUpdateConfigApi } from "./config.js";
...
export function createPackUpdateServiceApi(
  deps: PackUpdateDeps,
  config: PackUpdateConfigApi,
  bindings: PackUpdateBindingsApi
) {
  const { t, paint: c } = deps;
  const {
  ...
  const { getBinding, listBindings, packSourcesPath, setBinding } = bindings;
  ...
  function pairedRpUuidFromBpDir(bpDir: string): string | null {
    const depList = readPackDependencyUuids(bpDir);
    return depList[0] ?? null;
  }

  async function probeSourceAfterInstall(opts: {
    ...
    if (interactive && cfg.askConfirmOnBind && process.stdin.isTTY) {
      accept = await deps.askConfirm(t("packUpdate.confirmBind", { hit: best.hit.name }));
    } else {
  ...
  async function searchRemote(query: string): Promise<string> {
    const cfg = loadPackUpdateConfig();
    const provider = resolveConfiguredPackProvider(cfg);
    if (!provider) {
  ...
  async function bindPackSource(packId: string, ref: string): Promise<string> {
    const cfg = loadPackUpdateConfig();
    const provider = resolveConfiguredPackProvider(cfg);
    if (!provider) {
      return c.yellow(needKeyText());
    }
    const { bdsRoot, levelName } = deps.resolveBdsContext();
  ...
  function formatSourcesList(): string {
    ensurePackUpdateConfigFile();
    const bindingList = listBindings();
    const lines = [
    ...
    for (const { bpUuid, binding } of bindingList) {
  ...
  async function applyUpdate(r: CheckResult, cfg: PackUpdateConfig): Promise<{ ok: boolean; message: string }> {
    ...
    const oldRp =
      r.binding.pairedResourceUuid != null
        ? packs.find(
            (p) => p.kind === "resource" && p.uuid.toLowerCase() === r.binding.pairedResourceUuid!.toLowerCase()
          )
        : undefined;
    ...
    if (!rpInfo && rpRoots.length > 0) {
      const depList = readPackDependencyUuids(bpInstall.destDir);
      const fallback = await tryInstallResourcePack({
        rpRoots,
        bdsRoot,
        levelName,
        accept: (info) =>
          depList.length === 0 || depList.some((d) => d.toLowerCase() === info.uuid.toLowerCase()),
      });
      if (fallback) {
  ...
  async function checkPackUpdates(opts?: { packId?: string; apply?: boolean }): Promise<string> {
    const cfg = loadPackUpdateConfig();
  ...
  /** BDS 启动钩子：逐个检查并按配置应用 */
  async function runPackUpdatesOnBdsStart(): Promise<void> {
    const cfg = loadPackUpdateConfig();
  ...
  function bindingLabelForUuid(uuid: string): string {
    const b = getBinding(uuid);
    if (!b) return "src=-";
...
export type PackUpdateServiceApi = ReturnType<typeof createPackUpdateServiceApi>;
