...
export function createPackUpdateConfigApi(deps: Pick<PackUpdateDeps, "rootDir">) {
  const { rootDir } = deps;

  function packUpdateConfigPath(): string {
    return path.join(rootDir, "configs", "pack-update.json");
  }

  function loadPackUpdateConfig(): PackUpdateConfig {
  ...
  function getPackMatchConfig(cfg: PackUpdateConfig): PackUpdateMatchConfig {
    return cfg.match;
  }

  /** 卸载回收站绝对路径（相对 SFMC_ROOT；缺省走 DEFAULT_PACK_UNINSTALL） */
  function resolvePackTrashDir(cfg?: PackUpdateConfig): string {
  ...
  /** 是否使用卸载回收站 */
  function isPackUninstallRecycleBin(cfg?: PackUpdateConfig): boolean {
    const c = cfg ?? loadPackUpdateConfig();
   ...
   * 返回 null 表示直接删除。
   */
  function resolveUninstallTrashDir(opts?: {
    purge?: boolean;
   ...
   */
  function ensurePackUpdateConfigFile(): string {
...
export type PackUpdateConfigApi = ReturnType<typeof createPackUpdateConfigApi>;
