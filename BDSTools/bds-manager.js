...
let cfg = {};
try { cfg = JSON.parse(fs.readFileSync(CFG_PATH, 'utf-8')); } catch {}
...
function readPid() {
  try { return parseInt(fs.readFileSync(PID_FILE, 'utf-8').trim(), 10); } catch { return 0; }
}
  ...
  if (bdsProcess && bdsProcess.pid === pid && bdsProcess.stdin) {
    ...
    await Promise.race([
      ...
      new Promise((r) => setTimeout(() => {
        log('30s 超时，强制终止...');
        try { bdsProcess.kill('SIGTERM'); } catch {}
        setTimeout(r, 5000);
      }, 30000)),
    ]);
    ...
    if (bdsProcess && bdsProcess.exitCode === null) {
      log('强制结束进程...');
      try { bdsProcess.kill('SIGKILL'); } catch {}
    }
  } else {
...
if (require.main === module) {
  switch (cmd) {
    ...
    case 'stop':
      ...
      break;
    case 'restart':
      stop().then(() => start()).catch((e) => log(`重启失败: ${e.message}`));
      break;
    case 'status':
      status();
      break;
