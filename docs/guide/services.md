...
| `pack` status\|build\|deploy\|list\|… | 模块聚合行为包/资源包（见 [行为包](./behavior-pack.md)） |
| `packs`/`addon` list\|scan\|install\|… | 世界侧任意 BP/RP 收件箱（见 [世界资源包](./world-packs.md)） |
...
1. 扫描 `<SFMC_ROOT>/packs/` 收件箱，安装第三方世界 BP/RP（空收件箱无输出；见 [世界资源包](./world-packs.md)）
2. 比对世界 BP 内 `sfmc-deploy-catalog.json` 与本机模块启停/指纹
3. 不一致则整包 `build` + `deploy`（含 `world_*_packs.json` 与 `config/<uuid>/permission.json`）
