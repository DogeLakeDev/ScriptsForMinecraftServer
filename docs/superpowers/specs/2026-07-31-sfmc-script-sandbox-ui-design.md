...
- **夹具面板：** 当前 `moduleRoot` 的 id / enabled、settings JSON、权限覆盖、假 DB 清空；经 `fixture.get` / `fixture.apply`（重置保留意图）  
...
失败：**停在失败节点**（顶栏「定位失败」选中并 fitView）+ Output 完整日志；节点上钉短摘要。不自动清空世界（除非策略要求）。

**条件分支：** 断言出边 `data.kind`：`pass`（缺省，通过边）/ `fail`（失败边）。成功只走通过边可达子图；失败时若有失败边则转向其下游（再沿 pass），否则停在该断言。边右键或属性可切换类型。
...
后置：等待事件、新建 Entity、表单应答、分组框。断言失败边已交付（边 `data.kind=pass|fail`）。  
...
- RPC：`inspect` / `sceneSummary` / `create` / `emit` / `tick` / `reset` / `fixture.get` / `fixture.apply` 
