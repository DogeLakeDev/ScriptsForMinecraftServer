...
机制：[SDK · service](../sdk/service.md) · [服务 RPC](../services.md) · [manifest 契约](../dev/manifest.md)
...
仅 **消费** 服务、不对外 provides 的模块（如 `gui`、`chat`）——见各包 `sapi/manifest.json` 的 `services.requires`。
...
3. **装机**：`mod install` / `fetch-module` **不会**按 npm 依赖自动拉齐；装 `land` 时请同时装 `economy`。  
