...
用 **`npm create @sfmc-bds/module`** 建仓，再用 **VS Code/Cursor 扩展「SFMC Module」** 覆盖 Test → Link → Watch → Publish；运维面仍用 `sfmc`（装模块、启停、对本机 `mod reload`）。

一模块一仓；**不要** clone `sfmc-modules` 写业务。GitHub Template / `sfmc-new-module` / `rename.mjs` 已弃用。
...
交互选择：模块 id、显示名、npm scope（或官方包）、可选能力（如 `db`）。

也可在空窗口打开扩展后执行命令面板 → `SFMC: New Module`（与 CLI **同一** `createModule()` 引擎）。
...
1. **Create**：`npm create @sfmc-bds/module@latest` 或扩展 `SFMC: New Module`
2. **Test**：`npm test` / 扩展 `SFMC: Run Tests`（假引擎，秒级；不依赖 `sfmc.root`）。详情见 [测试沙箱](./testing.md)
3. **Link**：扩展 `SFMC: Link to SFMC Root`，或  
   `sfmc mod install <id> --from dir:<作者仓绝对路径> --link`
4. **Watch**：设 `sfmc.root` 为含 `configs/`、`modules/` 的工作目录 → `SFMC: Start Watch` / `Reload to BDS`
5. **Publish**：扩展 `SFMC: Publish to npm` 或 `npm publish --access public`，再向 `sfmc-modules` 的 `index.json` 开 PR。见 [发布你的模块](./publish.md)
...
| 入口 | 作用 |
| ------ | ------ |
| `npm create @sfmc-bds/module` | 建仓（唯一权威） |
| `npm test` / 扩展 Run Tests | 模块仓假引擎单测（`createSandbox`） |
| 扩展 Link | `mod install --from dir:… --link` |
| 扩展 Start Watch | 源码变更 → `@sfmc-bds/devkit` 重建部署 |
| `sfmc mod build` / `reload` | **运维**对本机已装模块组装部署 |
| `sfmc mod install/enable` | **运维**安装与启停 |

:::warning 注意
`sfmc mod test` / `mod watch` / `mod publish` 已移除；建仓只用 `npm create`，联调/发布用扩展全周期。
...
| `@sfmc-bds/create-module` | 建仓 API（扩展 / CLI） |
| `@sfmc-bds/devkit` | Watch / rebuild（扩展依赖） |
