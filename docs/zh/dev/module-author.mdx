...
本手册面向 SFMC 业务模块开发者。读完本篇，你将掌握从**新建模块仓库、规范化命名、本地假引擎单元测试、真机热重载联调，到最终发布至 npm 与官方索引**的完整工程化闭环。
...
向导将引导您填写并配置：
- **模块 ID（kebab-case）**：如 `teleport` 或 `daily-reward`
- **显示名称（Display Name）**：如 `领地传送` 或 `每日签到`
...
在 `sapi/manifest.json` 中，除了基础的模块 ID 和依赖外，推荐声明 **v3 语义化元数据（`semantic` 块）**。它能够帮助编辑器、静态沙箱与运维面板深刻理解该模块的行为特征：
...
- 🧪 **单元测试沙箱**：查阅 [测试沙箱指南](./testing.md) 了解如何编写高质量的 Mock 测试用例。
