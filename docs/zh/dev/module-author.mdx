...
2. **Test**：`pnpm test` / `npm test` / 扩展 `SFMC: Run Tests`（假引擎，秒级；不依赖 `sfmc.root`）。详情见 [测试沙箱](./testing.md)
...
5. **Publish**：扩展 `SFMC: Publish to npm` 或 `pnpm publish --access public` / `npm publish --access public`，再向 `sfmc-modules` 的 `index.json` 开 PR。见 [发布你的模块](./publish.md)
...
| `pnpm test` / `npm test` / 扩展 Run Tests   | 模块仓假引擎单测（`createSandbox`）    |
...
| 章节                         | 内容                             |
| ---------------------------- | -------------------------------- |
| [API 速查手册](./cheatsheet.mdx) | 常用生命周期、Msg、DB、配置与服务调用代码片段 |
