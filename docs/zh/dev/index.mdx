...
| 包管理 | **pnpm** workspace（`pnpm-workspace.yaml`，主仓推荐）或 **npm workspaces**；本地 `@sfmc-bds/*` 禁止 `workspace:*` |
...
:::warning 注意
`sfmc mod test` / `watch` / `publish` 已移除。测试与 Watch 用扩展；发布用 `pnpm publish` / `npm publish`，见 [发布你的模块](./publish.md)。
...
1. 克隆本仓 → `pnpm install` / `npm install` → `pnpm run build` / `npm run build --workspaces --if-present`  
...
cd <id> && pnpm install && pnpm test
cd <id> && npm install && npm test
# 联调：扩展「SFMC Module」→ Link to SFMC Root → Start Watch（设 sfmc.root）
# 或运维机：
...
| [API 速查手册](./cheatsheet.mdx) | 生命周期、Msg、DB、配置与服务调用 |
