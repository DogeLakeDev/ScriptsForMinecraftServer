...
| :--- | :--- | :--- |
| **作者独立仓（Template）** | **业务逻辑的唯一编码面**。每个模块均为独立的 Git 仓库，拥有自己的 `package.json`、构建与测试流程。 | 严禁直接在主仓 `packages/` 目录下添加具体业务逻辑。 |
...
- **SFMC Module**（官方专属扩展）：提供一键生成新模块（`New Module`）、软链接挂接（`Link`）、实时监听热重载（`Start Watch`）与一键部署。
- **ESLint**：启用 `@sfmc-bds/eslint-plugin` 规则，实施消息规范、SDK 导入路径与跨模块契约的静态检查。
- **终端与调试器**：运行类型检查、构建与平台自检，并查看服务日志。
...
3. [验证与调试指南](./testing.md) — 了解静态检查、平台自检与 BDS 联调
...
| **[验证与调试策略](./testing.md)** | 模块作者 Watch 热联调、平台自检与真机验收 |
