...
| 资产目录 / 文件 | 存储内容 | 丢失影响 |
| :--- | :--- | :--- |
| **`data/`** | SQLite 核心数据库文件（`sfmc_data.db` 及日志） | **极高**。包含所有玩家经济余额、签到历史、领地坐标与模块持久化数据。 |
...
### ② 升级 Bedrock Dedicated Server 官方核心
...
- `server.properties`（服务端网络与基础配置）
- `whitelist.json` / `permissions.json`（原生白名单与权限表）
