...
| :------: | :---------------------------- | :----------------------------------- | :------------------------------------------- |
|   `0`    | **Any**（游客）               | 所有进服玩家均持有的基础权限         | `/c:help`、`/c:ping`、`/c:menu`、`/c:online` |
...
模块包通过 `configs-default/<configKey>.json` 携带默认值。安装、更新或 link 时，CLI 会创建工作目录配置，或递归补入新增字段；用户已有值、数组和自定义字段不会被覆盖。
