...
先选症状逐步排查；完整清单仍保留在下方，便于检索与 AI 阅读。
...
| Testing 面板发现不了用例 | 装 `nodejs-testing`；核对模板 `.vscode/settings.json` 与 `pnpm test` / `npm test` 脚本 |

作者流程见 [测试沙箱](../dev/testing.md)。

### QQ 桥

**官方后端（`qq_backend: official`）**

1. `qq_app_id` / `qq_app_secret` 是否正确
2. 是否已订阅 `GROUP_AT_MESSAGE_CREATE`，群内是否 @ 机器人
3. `qq_group_openid` 是否已从日志抄入并重启 db / qq
4. 群主是否允许机器人主动发言（MC→QQ）
5. 沙箱是否与 `qq_sandbox` 一致；IP 白名单是否包含本机公网 IP

**LLBot 后端（`qq_backend: llbot`）**
...
从旧版升级未写 `qq_backend` 时默认走官方；若仍用 LLBot 请显式设 `"qq_backend": "llbot"`。
...
仍解决不了时，可前往 GitHub Issues 附上 BDS / sfmc 日志、`pnpm run verify` / `npm run verify` 输出和 Node 版本。
