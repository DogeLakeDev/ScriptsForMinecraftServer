...
`db.tx` 走交互会话；回调内 `await tx.*` / `tx.call` 返回真实结果，可据此分支。
...
await db.tx(async (tx: TxContext) => {
  ...
  const debit = await tx.call<{ ok: boolean }>("economy.debit", {
    playerId,
    amount: 100,
  });
...
表达式树（如 `{ field, op, value }`）。禁止 SQL 字符串。

## 错误

`DbError` 含平台错误码；权限不足、表冲突等由 HTTP 层返回 4xx。
