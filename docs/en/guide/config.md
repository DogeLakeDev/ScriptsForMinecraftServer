...
Each module declares `configKey` in `sapi/manifest.json`, mapped to `configs/<configKey>.json`. The module package supplies defaults in `configs-default/<configKey>.json`; install, update, and link operations create the working configuration or recursively add missing fields without overwriting existing user values. Runtime reads and writes go through `@sfmc-bds/sdk/sapi/config` (HTTP: `/api/sfmc/configs/:configKey`), separate from platform `ConfigName`.
