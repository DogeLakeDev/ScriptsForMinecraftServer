...
Scaffold from [sfmc-module-template](https://github.com/Tanya7z/sfmc-module-template), then at the repo root:

```bash
node scripts/rename.mjs my-feature --scope <npm-user> --name "My Feature"
npm i && npm test
```

Link into your SFMC workspace (module short id required). Authors use extension Watch; on the server:
...
| [Remote control](./remote.md) | `remote enroll` and controller |
