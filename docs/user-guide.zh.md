...
```bash
# 配置由 sfmc wizard 首次运行写入 configs/*.json,无需手工 cp
# 手工覆盖:复制 configs/.env.example 为 configs/.env 后再编辑
```
...
```bash
sfmc behavior-pack build      # 等价于 npm 下的 esbuild + pack-manager
sfmc behavior-pack deploy     # 写到 BDS worlds/<level>/behavior_packs/sfmc-modules/
```
