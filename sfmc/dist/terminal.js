...
function readTerminalSettings(file) {
    try {
        const errors = [];
        const settings = parse(fs.readFileSync(file, "utf-8"), errors);
        if (errors.length === 0)
            return settings;
...
function terminalSettingsFiles() {
...
function preferredPowerShell() {
    const programFiles = [process.env.ProgramW6432, process.env.ProgramFiles].filter((value) => Boolean(value));
    const candidates = [
...
function profile(guid, startingDirectory, commandline) {
    return {
        altGrAliasing: true,
...
function removeManagedProfiles(settingsFile, settings, state) {
...
function updateProfile(settingsFile, state, commandline) {
...
function terminalState(runtimeFile, startingDirectory) {
    const runtime = readJson(runtimeFile, {});
    const saved = runtime.terminal_profile;
    const changed = saved?.starting_directory !== startingDirectory || !saved.guid;
    if (changed && saved?.guid) {
        for (const settingsFile of terminalSettingsFiles())
            removeProfile(settingsFile, saved);
    }
    const state = changed
...
function windowsTerminalAvailable() {
...
export function ensureSeaTerminalProfile() {
    if (!IS_SEA || process.platform !== "win32" || process.env.SFMC_SERVICE)
        return;
    const settingsFiles = terminalSettingsFiles();
    if (settingsFiles.length === 0 || !windowsTerminalAvailable())
        return;
    const runtimeFile = configPath(ROOT, "runtime.json");
    const state = terminalState(runtimeFile, path.dirname(process.execPath));
    for (const settingsFile of settingsFiles)
        updateProfile(settingsFile, state, preferredPowerShell());
}
