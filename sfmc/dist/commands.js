...
import { ROOT, SERVICE_NAMES, services } from "./services.js";
import { c, DIVIDER, highlightLogLine } from "./theme.js";
function parseService(raw) {
    const s = raw.toLowerCase();
    if (SERVICE_NAMES.includes(s))
        return s;
    return null;
}
...
export function cmdLogs(args, onFollow) {
    let n = 20;
    let follow = false;
    const positional = [];
    for (const a of args) {
    ...
    if (nIdx >= 0 && nIdx + 1 < args.length)
        n = parseInt(args[nIdx + 1] ?? "0", 10);
    const svcRaw = positional[0];
    if (!svcRaw)
        return c.yellow("Usage: logs <service> [-n N] [-f]");
    const svc = parseService(svcRaw);
    if (!svc)
        return c.red(`Unknown service: ${svcRaw} (try: ${SERVICE_NAMES.join(", ")})`);
    const svcObj = services[svc];
    const lines = svcObj.getRecentLogs(n);
    if (lines.length === 0)
        return c.dim("(no logs yet)");
...
export async function cmdStart(raw) {
    const svc = parseService(raw);
    if (!svc)
        return c.red(`Unknown service: ${raw} (try: ${SERVICE_NAMES.join(", ")})`);
    const svcObj = services[svc];
    if (svcObj.running)
        return c.yellow(`${svcObj.title} already running (PID ${svcObj.pid})`);
    if (STARTING.has(svc))
        return c.dim(`${svcObj.title} already starting...`);
    STARTING.add(svc);
    try {
        await svcObj.start();
...
export async function cmdStop(raw) {
    const svc = parseService(raw);
    if (!svc)
        return c.red(`Unknown service: ${raw} (try: ${SERVICE_NAMES.join(", ")})`);
    const svcObj = services[svc];
    if (!svcObj.running)
        return c.yellow(`${svcObj.title} already stopped`);
    if (STOPPING.has(svc))
        return c.dim(`${svcObj.title} already stopping...`);
    STOPPING.add(svc);
    try {
        await svcObj.stop();
...
export async function cmdSend(raw, message) {
    const svc = parseService(raw);
    if (!svc)
        return c.red(`Unknown service: ${raw} (try: ${SERVICE_NAMES.join(", ")})`);
    if (!message)
        return c.yellow("Usage: send <service> <message>");
    const svcObj = services[svc];
    if (!svcObj.running || !svcObj.proc?.stdin)
        return c.yellow(`${svcObj.title} not running`);
    try {
...
export async function cmdRestart(raw) {
    const svc = parseService(raw);
    if (!svc)
        return c.red(`Unknown service: ${raw} (try: ${SERVICE_NAMES.join(", ")})`);
    const svcObj = services[svc];
