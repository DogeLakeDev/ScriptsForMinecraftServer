...
const defaultRoot = IS_SEA ? path.dirname(process.execPath) : process.cwd();
export const ROOT = resolveRuntimeRoot(defaultRoot);
export const PACKAGES_DIR = path.join(ROOT, "modules", "packages");
 ...
 */
export function spawnService(service, args = [], opts = {}) {
    const env = {
        ...process.env,
        ...opts.env,
...
export function spawnServiceSync(service, args = [], opts = {}) {
    const env = {
        ...process.env,
        ...opts.env,
        SFMC_SERVICE: service,
