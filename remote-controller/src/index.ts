...
type TaskAction = "status" | "start" | "stop" | "restart";
type Task = {
  id: string;
  agentId: string;
  action: TaskAction;
...
const server = createServer(async (req, res) => {
  ...
  try {
    if (req.method === "POST" && url.pathname === "/v1/enroll") {
      if (!enrollmentToken || !authorized(req, enrollmentToken)) return json(res, 401, { error: "unauthorized" });
    ...
    if (!adminToken || !authorized(req, adminToken)) return json(res, 401, { error: "unauthorized" });
    ...
    if (req.method === "POST" && taskMatch?.[1]) {
      ...
      if (input.action !== "status" && typeof input.service !== "string") return json(res, 400, { error: "service_required" });
      if (typeof input.service === "string" && !serviceNames.has(input.service)) return json(res, 400, { error: "invalid_service" });
      const task: Task = {
        id: randomUUID(),
        agentId,
        action: input.action,
...
wss.on("connection", (socket: WebSocket, req: IncomingMessage, agentId: string) => {
  ...
  socket.on("message", (raw: RawData) => {
    ...
    try {
      const message = JSON.parse(raw.toString()) as { type?: string; taskId?: string; ok?: boolean; result?: unknown; error?: string };
