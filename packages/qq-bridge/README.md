...
云端桥需要访问游戏机 db-server 的多个本地 API。db-server 只监听游戏机 `127.0.0.1:3001`，不要开放其端口。游戏机 SFMC CLI 的 `tunnel` 服务主动建立 SSH 反向隧道，将云机 `127.0.0.1:13001` 转发到游戏机 `127.0.0.1:3001`；云端配置 `db_host: "127.0.0.1"`、`db_port: 13001`。在 `qq_config.json` 的 `official.tunnel` 配置 `{ "enabled": true, "host": "aliyun", "remotePort": 13001 }`，其中 `host` 是本机 SSH 配置中的别名。SFMC 启动 db-server 后启动隧道，异常退出时重试，并通过 `status`、`start tunnel`、`stop tunnel`、`restart tunnel` 和 `logs tunnel` 管理。远端只监听回环地址；不要同时运行旧的 `deploy/qq-tunnel.ps1`，否则两者会争抢云端端口。配置同步任务仍负责同步云端机器人配置，与隧道的生命周期分开。
...
- 解绑等需确认的操作：先显示对象与影响，60 秒内本人确认；取消、新命令或超时后失效。确认时重新核验权限，重复确认不重复提交。
