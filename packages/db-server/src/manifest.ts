 ...
 *      (通常与 SFMC_ROOT 对齐)
 *   2. PROJECT_ROOT/modules/packages —— 独立启动时兜底
 */
export function defaultPackagesDir(): string {
  return process.env.SFMC_PACKAGES_DIR || join(PROJECT_ROOT, "modules", "packages");
}
...
export function loadManifest(packagesDir: string = defaultPackagesDir()): ModuleManifest {
  ...
  const ids = readdirSync(packagesDir, { withFileTypes: true })
    ...
    .sort();
  for (const id of ids) {
    const manifestPath = join(packagesDir, id, "sapi", "manifest.json");
