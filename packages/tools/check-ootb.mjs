...
import { CONFIGS_DIR, DB_SERVER_DIST, FETCH_MODULE, ROOT, SFMC_DIST, TOOLS_PKG_DIR } from "./lib/paths.mjs";
...
async function main() {
  ...
  // 2) 配置 schema + ensure 播种后 configs 必须齐全
  {
    ...
    const seed = runSync(process.execPath, [path.join(TOOLS_PKG_DIR, "seed-configs.mjs")], {
      cwd: ROOT,
  ...
  {
    const sync = runSync(process.execPath, [path.join(TOOLS_PKG_DIR, "catalog-sync.mjs")], { cwd: ROOT });
    if (sync.status === 0) pass("catalog-sync 通过");
    else fail("catalog-sync 通过", (sync.stderr || sync.stdout || `exit ${sync.status}`).trim());

    const r = runSync(process.execPath, [path.join(TOOLS_PKG_DIR, "check-modules.mjs")], { cwd: ROOT });
  ...
  {
    const r = runSync(process.execPath, [path.join(TOOLS_PKG_DIR, "sim-new-user.mjs")], {
      cwd: ROOT,
      timeout: 90000,
    });
  ...
  {
    if (!exists(SFMC_DIST)) {
      console.log("[ootb] WARN: packages/cli/dist/main.js 缺失 — 跳过 CLI 检查");
    } else {
