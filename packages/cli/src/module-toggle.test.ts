/**
 * module-toggle 本地写 lock + 通知结果信封（纯函数）
 */
import assert from "node:assert/strict";
import test from "node:test";
import { applyLockEnabled, finalizeToggle, resolveToggleTarget } from "./module-toggle.js";
...
test("applyLockEnabled：写入 enabled + updatedAt", () => {
  const lock = { version: 1, modules: {} as Record<string, { enabled: boolean; updatedAt?: number }> };
