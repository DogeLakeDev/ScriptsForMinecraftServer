/**
 * packs uninstall 策略契约：受保护平台包识别 + 多 id 解析
 */
import assert from "node:assert/strict";
import { describe, it } from "node:test";
import { isProtectedSfmcPack, resolveUninstallTargets, type InstalledWorldPack } from "./world-packs.js";

function fakePack(folderName: string, uuid = `uuid-${folderName}`): InstalledWorldPack {
  return {
    folderName,
    uuid,
    name: folderName,
...
describe("resolveUninstallTargets LSP", () => {
  ...
  it("单 id 受保护 → protected", () => {
    ...
    if (r.status === "protected") assert.equal(r.folder, "sfmc-modules");
  });
  ...
  it("多 id 均可卸 → ok，按 uuid 去重", () => {
    ...
    if (r.status === "ok") assert.equal(r.selected.length, 2);
  });

  it("缺 id → not_found", () => {
    const r = resolveUninstallTargets(packs, ["nope", "[BP] Addon"]);
    assert.equal(r.status, "not_found");
    if (r.status === "not_found") assert.deepEqual(r.missing, ["nope"]);
  });
});
