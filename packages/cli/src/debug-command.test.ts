/**
 * debug-command BDS variables.json / secrets.json 读写
 */
...
import { cmdDebug, isDebugTruthy } from "./debug-command.js";

function setupSfmcRoot(bdsRoot: string): string {
...
test("debug enable/disable/status 写 BDS variables.json", async () => {
  ...
  try {
    ...
    const afterEnable = JSON.parse(fs.readFileSync(variablesPath, "utf8")) as { sfmc_debug?: boolean };
    ...
    const afterDisable = JSON.parse(fs.readFileSync(variablesPath, "utf8")) as { sfmc_debug?: boolean };
...
test("debug sentry on/off 写 BDS secrets.json", async () => {
  ...
  try {
    ...
    const afterOn = JSON.parse(fs.readFileSync(secretsPath, "utf8")) as { SENTRY_DSN?: string };
    assert.equal(afterOn.SENTRY_DSN, dsn);

    await cmdDebug(["sentry", "off"]);
    const afterOff = JSON.parse(fs.readFileSync(secretsPath, "utf8")) as Record<string, unknown>;
