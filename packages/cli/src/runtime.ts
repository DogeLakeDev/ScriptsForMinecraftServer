...
/**
 * 是否为 monorepo 布局（仓根同时有 packages/db-server 与 packages/cli）。
 * npm 全局/本地安装时 ROOT 通常是 cwd,不满足此条件。
 */
export function isMonorepoLayout(root: string): boolean {
  return (
    fs.existsSync(path.join(root, "packages", "db-server", "package.json")) &&
    fs.existsSync(path.join(root, "packages", "cli", "package.json"))
  );
}
 ...
 * - monorepo: walk-up 到 sfmc-monorepo
...
const SERVICE_SCRIPT: Record<ServiceId, string> = {
  db: "packages/db-server/dist/index.js",
  qq: "packages/qq-bridge/dist/index.js",
  update: "packages/bds-tools/dist/check-update.js",
  manager: "packages/bds-tools/dist/bds-manager.js",
  "pack-manager": "packages/bds-tools/dist/cli-pack-manager.js",
};
...
export function resolveSdkPackageRoot(): string {
  ...
  // SFMC_ROOT 可能是数据目录；再从 CLI 位置 walk-up monorepo
  const monoFromCli = findMonorepoRoot(__dirname);
  if (monoFromCli) {
    const fromCli = path.join(monoFromCli, "modules", "sdk", "@sfmc-sdk");
