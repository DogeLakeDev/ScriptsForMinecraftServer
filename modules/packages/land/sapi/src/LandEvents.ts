...
export class LandEvents {
  ...
  private static themeColorFor(land: any): { particle: string; hex: string } {
    ...
    if (!particle) return { particle: palette[0]!, hex: "#" + ((safeIds * 0x9e3779b1) >>> 0).toString(16).padStart(8, "0").slice(0, 6) };
    return {
      particle,
      hex: "#" + ((safeIds * 0x9e3779b1) >>> 0).toString(16).padStart(8, "0").slice(0, 6),
    };
  }
