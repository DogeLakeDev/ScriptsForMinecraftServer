...
import { LandCore, Database, canManage, getPlayerRole, LandData, LandPos, LandRole } from "@sfmc/module-land";
import { debug } from "@sfmc/sdk/sapi/runtime";
import { Money } from "@sfmc/sdk/sapi/runtime";
import { FormStatus, MenuNavigator, obsBool, obsNum, obsStr } from "@sfmc/sdk/sapi/runtime";
import { dimensionId, ListFormInfo, Msg } from "@sfmc/sdk/sapi/runtime";
...
export class LandGUI {
  ...
  private async refreshPreviewPrice(
    ...
    try {
      const { validateLand } = await import("./LandApi.js");
    ...
    page.button(
      ...
      () =>
        void this.nav.runTask(status, async () => {
          ...
          const { transferLand } = await import("./LandApi.js");
